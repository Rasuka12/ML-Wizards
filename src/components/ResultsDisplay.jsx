      ...
      {showAdvancedResults && advancedSearchResults && (
        <div className="bg-gradient-to-r from-blue-50 to-cyan-50 border-2 border-blue-200 rounded-2xl p-6">
          ...
          <div className="mb-6 p-4 bg-white rounded-xl border border-blue-200 shadow-sm">
            <h6 className="text-sm font-semibold text-blue-800 mb-3">🔍 Recommended Search Queries:</h6>
            <div className="space-y-2">
              <div className="p-2 bg-blue-50 rounded-lg border border-blue-100">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-blue-600 font-medium">Primary:</span>
                  <button
                    onClick={() => copyQueryToClipboard(advancedSearchResults.searchQuery, 'primary')}
                    className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded hover:bg-blue-200 transition-colors"
                  >
                    {copiedQuery === 'primary' ? '✓ Copied!' : 'Copy'}
                  </button>
                </div>
                <p className="text-sm text-gray-800 font-medium">{advancedSearchResults.searchQuery}</p>
              </div>
              {advancedSearchResults.alternativeQueries && advancedSearchResults.alternativeQueries.length > 0 && (
                <div>
                  <span className="text-xs text-blue-600 font-medium">Alternatives:</span>
                  <div className="grid gap-1 mt-1">
                    {advancedSearchResults.alternativeQueries.map((query, index) => (
                      <div key={index} className="p-2 bg-gray-50 rounded border">
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-700 flex-1">{query}</span>
                          <button
                            onClick={() => copyQueryToClipboard(query, `alt-${index}`)}
                            className="ml-2 text-xs bg-gray-200 text-gray-600 px-2 py-1 rounded hover:bg-gray-300 transition-colors"
                          >
                            {copiedQuery === `alt-${index}` ? '✓' : 'Copy'}
                          </button>
                        </div>
                      </div>
                    ))}
          ...
          {/* Search Strategies */}
          {advancedSearchResults.searchStrategies && advancedSearchResults.searchStrategies.length > 0 && (
            <div className="mb-6 p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-200 shadow-sm">
              <h6 className="text-sm font-semibold text-green-800 mb-3 flex items-center space-x-1">
                <span>🎯</span>
                <span>Platform-Specific Search Strategies:</span>
              </h6>
              <div className="space-y-3">
                {advancedSearchResults.searchStrategies.map((strategy, index) => (
                  <div key={index} className="bg-white p-3 rounded-lg border border-green-100">
                    <div className="flex items-center justify-between mb-2">
                      <h7 className="font-semibold text-green-900 text-sm">{strategy.platform}</h7>
                      <button
                        onClick={() => copyQueryToClipboard(strategy.query, `strategy-${index}`)}
                        className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded hover:bg-green-200 transition-colors"
                      >
                        {copiedQuery === `strategy-${index}` ? '✓ Copied!' : 'Copy Query'}
                      </button>
                    </div>
                    <div className="mb-2">
                      <span className="text-xs text-green-600 font-medium">Search Terms:</span>
                      <code className="block text-xs bg-gray-100 text-gray-800 p-1 rounded mt-1 font-mono">
                        {strategy.query}
                      </code>
                    </div>
                    <div>
                      <span className="text-xs text-green-600 font-medium">Instructions:</span>
                      <p className="text-xs text-gray-600 mt-1">{strategy.instructions}</p>
                    </div>
                  </div>
                ))}
              </div>
